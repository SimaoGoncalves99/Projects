...
def validate_source(dataloader, F, C, loss_C, optimizer_F, optimizer_C, epoch, best_model_metric):
...
def validate_target(dataloader, F, C, loss_C, optimizer_F, optimizer_C, epoch, best_model_metric):
    """
...
def compute_covariance(input_data):
    ...
    term_mult = (-1/n)*torch.mm(sum_column.t(), sum_column)
    d_t_d = torch.mm(input_data.t(), input_data)
    add = torch.add(d_t_d,term_mult)
    c = (1/(n-1))*add
